...
namespace UP_02_Glebov_Drachev.Views.Pages
{
    ...
    public partial class Login : Page
    {
        ...
        {
            //DBConnection.CreateConnection("admin", "admin123");
            //MainWindow.OpenPages(new Pages.GeneralPage());
            if (!string.IsNullOrEmpty(LoginTextBox.Text) || !string.IsNullOrEmpty(PasswordTextBox.RealPassword))
            {
                if (DBConnection.CreateConnection(LoginTextBox.Text, PasswordTextBox.RealPassword) == null)
                {
                    return;
                }
                else
